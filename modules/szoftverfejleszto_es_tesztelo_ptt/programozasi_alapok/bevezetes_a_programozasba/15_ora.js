let i = 1;
let aktiv = '15. óra';

let adatok = [
    {
        anyag: [
            ['h1', `${i++}. Kártya`],
            ['h2', 'A tanulási terület tartalmi összefoglalója'],
            ['img', '../../programozasi_alapok/images/kep_001.jpg'],
            ['p', 'Ismerkedés a Codecademy oktatási portállal.'],
            [
                'a',
                'https://api.ikk.hu/storage/uploads/files/ptt_informatika_szoftverfejleszto_es_-tesztelo_2020pdf-1599123461507.pdf',
                'PTT - Programtanterv',
            ],
        ],
    },
    {
        anyag: [
            ['h1', `${i++}. Kártya`],
            ['h3', 'Játékos feladatok a Codecademy oktatási portálon'],
            ['h4', 'Codecademy'],
            ['img', '../../programozasi_alapok/images/kep_011.jpg'],
            ['a', 'https://www.codecademy.com/', 'Codecademy'],
        ],
    },
];
